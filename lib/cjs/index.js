...
if ((DataView$2 && getTag$3(new DataView$2(new ArrayBuffer(1))) != dataViewTag$2$1) ||
    ...
    (WeakMap$2 && getTag$3(new WeakMap$2) != weakMapTag$1$1)) {
...
function trimEnd(string, chars, guard) {
  ...
  var strSymbols = stringToArray$1(string),
      end = charsEndIndex(strSymbols, stringToArray$1(chars)) + 1;
...
class AxiosException extends axios.AxiosError {
    __CANCEL__;
    __NETWORK_ERROR__;
}
const DEFAULT_LOG = 'none';
...
const prepareAxios = (config, options) => {
    ...
    const prepareOptions = { ...DEFAULT_OPTIONS, ...options, testSignal };
    ...
    const start = (overrideOptions) => {
        const startOptions = { ...prepareOptions, ...overrideOptions };
...
const begin = async (config, options) => {
    ...
    if (options.test) {
        ...
        if (options.testSleep) {
            await new Promise((resolve) => {
                const resolveThis = () => {
                    options.testSignal.current = () => undefined;
                    resolve();
                };
                const timeout = setTimeout(resolveThis, options.testSleep);
                options.testSignal.current = () => {
                    testCancelled = true;
        ...
        const error = new AxiosException('Test', 'TEST', config, null, response);
        ...
        try {
            response = await (options.axios ?? axios__default["default"])(config);
        }
        catch (e) {
            return parseError(e, config, options, startTime);
        }
    }
...
const parseError = async (error, config, options, startTime) => {
    ...
    const errorResponse = {
        ...
        error,
        isAxiosError,
        isCancel: (options.axios ?? axios__default["default"]).isCancel(error),
        isError: true,
...
const formatFullUrl = (config) => {
    return filter_1([trimEnd_1(config.baseURL, '/'), trimStart_1(config.url, '/')]).join('/') || undefined;
};

exports.prepareAxios = prepareAxios;
